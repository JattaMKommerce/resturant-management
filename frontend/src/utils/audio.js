...
export function playServiceChime(type = 'call_waiter') {
  try {
    ...
    // ONLY play audio chime for logged-in staff/admin users
    ...
    if (!token && !userStr) {
      return; // Silent for unauthenticated users
    }
    ...
    if (type === 'call_waiter' || type === 'chime') {
      ...
      harmGain1.gain.setValueAtTime(0.18, now);
      ...
      gain2.gain.setValueAtTime(0.40, strike2Time);
      gain2.gain.exponentialRampToValueAtTime(0.001, strike2Time + 1.4);
      harmGain2.gain.setValueAtTime(0.20, strike2Time);
      ...
      const notes = [
        { freq: 523.25, time: 0.00, dur: 0.40, gain: 0.25 }, // C5
        { freq: 659.25, time: 0.12, dur: 0.40, gain: 0.28 }, // E5
        { freq: 783.99, time: 0.24, dur: 0.50, gain: 0.32 }, // G5
        { freq: 1046.50, time: 0.36, dur: 1.20, gain: 0.38 } // C6 (Sustained finish)
      ];
