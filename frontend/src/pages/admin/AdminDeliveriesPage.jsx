          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {activeDeliveries.map((ord) => (
                <div key={ord.id} className="p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="font-mono text-xs font-bold text-slate-900">#{ord.order_number}</span>
                      <h3 className="font-bold text-sm text-slate-800">{ord.customer_name}</h3>
                      <p className="text-xs text-slate-500 leading-relaxed">{ord.delivery_address}</p>
                    </div>
                  ...
                  <div className="p-3 bg-white rounded-xl border border-slate-200 text-xs flex justify-between items-center">
                    <div>
                      ...
                      <span className="text-[11px] text-slate-500 block">({ord.vehicle_number})</span>
                    </div>
                    <span className="font-black text-emerald-600 text-sm">₹{ord.total_amount} ({ord.payment_method})</span>
                  </div>
